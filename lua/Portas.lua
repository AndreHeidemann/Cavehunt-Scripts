...
function openDoor(doorID,searchSize)    
    ...
    for x = -searchSize, searchSize do
        for y = -searchSize, searchSize do
            local tileItems = Map.getThings(playerPos.x + x, playerPos.y + y, playerPos.z)
            if tileItems then
                for _, item in ipairs(tileItems) do
                    if item.id == doorID then
                        doorX, doorY, doorZ = playerPos.x + x, playerPos.y + y, playerPos.z
                        print("Door with ID " .. doorID .. " found on screen.")
                        print("Position - X: " .. doorX .. ", Y: " .. doorY .. ", Z: " .. doorZ)
                        break
                    end
                end
            end
    ...
    if doorX then
        ...
        print("Opening the door at position - X: " .. doorX .. ", Y: " .. doorY .. ", Z: " .. doorZ)
    else
