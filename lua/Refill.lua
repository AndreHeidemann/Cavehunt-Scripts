...
function buyItems(item)
    ...
    if neededCount > 0 then
        ...
        if success then
            showMessage("Compradas: " .. neededCount .. " " .. item.name .. " (ID " .. item.id .. ")")
        else
            showMessage("Erro ao comprar: " .. neededCount .. " " .. item.name .. " (ID " .. item.id .. ")")
            return  -- Retorna para evitar a mensagem "Compra de suprimentos concluída"
        end
    else
        showMessage("Quantidade suficiente de " .. item.name .. " (ID " .. item.id .. ")")
    end
end
